...
def match_and_sum_shap(raw_features, shap_contribs):
    SPECIALS = {"salaire_cat", "salaire_cat_eq", "position_salaire_poste", "position_salaire_poste_anc"}
    lines = []
...
def filter_ids(search):
    ...
    filtered = [i for i in all_ids if search in i.lower()] if all_ids and search else all_ids
...
def predict_attrition(manual_id):
    try:
        ...
        return format_pred_string(f"Erreur ID sélectionné : {e}"), "-", None, pd.DataFrame()
    try:
        resp = requests.get(
            FASTAPI_URL,
        ...
        if resp.status_code != 200:
            return format_pred_string(f"Erreur API statut {resp.status_code}"), "-", None, pd.DataFrame()
        res = resp.json()
    except Exception as e:
        return format_pred_string(f"Erreur d'appel API : {e}"), "-", None, pd.DataFrame()
    if res.get("error"):
...
def check_health():
    ...
    return (
        banner,
        gr.Textbox(interactive=interactive, label="Filtrer la liste d'IDs salariés", placeholder="Ex: 14"),
        gr.Dataframe(value=df_value, headers=["id_employee"], label="Liste filtrée (30 max)", interactive=False),
        gr.Textbox(interactive=interactive, label="ID à prédire (copier/coller depuis la table)", placeholder="Ex: 1427"),
...
with gr.Blocks() as demo:
    ...
    with gr.Row():
        search_box = gr.Textbox(label="Filtrer la liste d'IDs salariés", placeholder="Ex: 14", interactive=True)
    ...
    with gr.Row():
        id_textbox = gr.Textbox(label="ID à prédire (copier/coller depuis la table)", placeholder="Ex: 1427", interactive=True)
